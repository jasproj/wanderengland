...
const CITY_PICKER = window.CityPicker ? window.CityPicker.create({
    cityOf: t => {
        ...
        return ({ 'Brighton and Hove': 'Brighton' })[n] || n;
    },
    // old options were london/liverpool/oxford/cambridge on the sparse `island`
    // field (set on 372 of 1,424 rows); match the town too so "London" finds
    // all London tours, not the 139 that carried the field
    legacyMatch: (t, v) => (t.island || '').toLowerCase() === v
        || window.CityPicker.lastSegment(t.location).toLowerCase() === v,
    minCount: 3,
    allLabel: 'All Towns & Cities'
}) : null;
